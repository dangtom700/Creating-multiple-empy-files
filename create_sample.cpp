...
void createCondenseCalendar(const std::string& mainRoot){
    const int currentYear=get_current_year();
    for (int year = 2015; year <= currentYear; year++){
        std::string filename=mainRoot+"/Advent_of_code"+std::to_string(year);
        std::ofstream file(filename+".cpp");
        file.close();
        std::string directory = mainRoot+"/Advent_of_code_"+std::to_string(year)+"_text_files";
        create_dir(directory);
        for (int date = 1; date<=25; date++){
            std::string text_file="/Advent_of_code_"+std::to_string(date)+".txt";
            std::ofstream file(directory+text_file);
            file.close();
        }
    }
}
int main(){
    ...
    const std::string mainRoot = "AdventOfCode_CPP";
    createCondenseCalendar(mainRoot);
    return 0;
}